...
function PurposeStep({
  ...
  return (
    <>
      ...
      <Heading headingRef={headingRef}>Plan Warped Tour with your crew</Heading>
      <p className="mt-2 text-[15px] leading-relaxed text-secondary">
        Pick the bands you want to see, compare plans with friends, find meetup windows, and
        keep everything available when cell service disappears.
      </p>

      <ul className="mt-6 space-y-3">
        <Benefit Icon={Star} title="Pick your bands" body="Must See, Want to See, Maybe — and spot clashes early." />
        <Benefit Icon={Users} title="Share plans with friends" body="Scan a code. No accounts, no signal needed." />
        <Benefit Icon={WifiOff} title="Use schedules and maps offline" body="Everything lives on this phone." />
      </ul>
...
/**
 * The whole app in three lines (plan §"Simple mental model"). Ari and Morgan
 * shouldn't have to work out what each tab is for — the app has three modes
 * and which one you're in depends only on what the festival has announced.
 */
export function PhaseModel({ className }: { className?: string }) {
  const phases: [string, string][] = [
    ['Before the festival', 'Pick bands and import your friends.'],
    ['When set times are released', 'Import the latest schedule from Robbie.'],
...
function PlanStep({
  ...
  return (
    <>
      <Heading headingRef={headingRef}>Add your festival plan</Heading>
      <p className="mt-2 text-[15px] text-secondary">
      ...
      <div className="mt-6 space-y-3">
        ...
        >
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent-soft text-accent">
            {isOrganizer ? <Download size={22} aria-hidden /> : <QrCode size={22} aria-hidden />}
          </span>
          <span className="flex-1">
            <span className="block font-display text-[17px] text-primary">
              {isOrganizer ? 'Import an Existing Backup' : 'Import From Robbie'}
            </span>
            <span className="block text-[13px] leading-snug text-secondary">
              {isOrganizer
                ? 'Restore a saved backup file or code from another phone.'
                : "Scan Robbie's schedule or group code to load the latest shared information."}
            </span>
          </span>
        </button>
      </div>
      ...
      <p className="mt-4 rounded-xl bg-[var(--surface-sunken)] px-3 py-2.5 text-[13px] leading-relaxed text-secondary">
        Importing Robbie&apos;s schedule will not replace your personal band choices.
      </p>
