...
describe('break planning (add-on §7)', () => {
  it('finds a food window in the gap between two sets', () => {
    ...
    const [food] = planBreaks('robbie', 'saturday', ['food'], ctx(perfs));
  ...
  it('reports nothing when no gap is long enough', () => {
    ...
    expect(planBreaks('robbie', 'saturday', ['food'], ctx(perfs))).toEqual([]);
  });
  ...
  it('a short need still fits where a long one does not', () => {
    ...
    const c = ctx(perfs);
    expect(planBreaks('robbie', 'saturday', ['food'], c)).toEqual([]);
    const [water] = planBreaks('robbie', 'saturday', ['water'], c);
  ...
  it('does not suggest a break that the walk would swallow', () => {
    ...
    const [ok] = planBreaks('robbie', 'saturday', ['water'], ctx(near));
  ...
  it('returns one window per requested need', () => {
    ...
    const out = planBreaks('robbie', 'saturday', ['food', 'water'], ctx(perfs));
