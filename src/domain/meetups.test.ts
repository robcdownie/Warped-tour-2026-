...
describe('meetup engine (spec §29)', () => {
  it('suggests a meetup in a shared 30+ minute opening (acceptance §37-38)', () => {
    // Robbie: set ends 15:00 at Vans; next at 16:00 at Vans → 60 min free.
    // Ari: set ends 15:00 at Vans; next at 16:00 at Vans → same window.
    const perfs = [
    ...
    const sels = [sel('robbie', 'r1'), sel('robbie', 'r2'), sel('ari', 'a1'), sel('ari', 'a2')];
    const meetups = findMeetups('saturday', makeCtx(perfs, sels));
    expect(meetups.length).toBeGreaterThan(0);
    const m = meetups[0];
    expect(m.userIds).toContain('robbie');
    expect(m.userIds).toContain('ari');
    expect(m.durationMinutes).toBeGreaterThanOrEqual(15);
  });

  it('never schedules a meetup on top of a Must-See set (acceptance §39-40)', () => {
    // Robbie has a must-see 15:00-15:45 at Ghost. Ari is free the whole time.
    const perfs = [
      perf('rMust', 'saturday', 'ghost', '15:00', '15:45'),
      perf('a1', 'saturday', 'rex', '12:00', '12:45'),
    ];
    const sels = [sel('robbie', 'rMust', 'must-see'), sel('ari', 'a1')];
    const meetups = findMeetups('saturday', makeCtx(perfs, sels));
    // No suggested meetup window should overlap Robbie's must-see 15:00-15:45.
    for (const m of meetups) {
      if (m.userIds.includes('robbie')) {
  ...
  it('considers the next stage when choosing where to meet (acceptance §41)', () => {
    // Robbie free until 16:00 at Ghost (east). Ari free until 16:00 at Rex (west).
    ...
    const sels = [sel('robbie', 'r2'), sel('ari', 'a2'), sel('morgan', 'r2')];
    const meetups = findMeetups('saturday', makeCtx(perfs, sels));
    expect(meetups.length).toBeGreaterThan(0);
    const m = meetups[0];
    const robbiePlan = m.perUser.find((p) => p.userId === 'robbie');
    expect(robbiePlan?.nextStageId).toBe('ghost');
    expect(robbiePlan?.leaveByMinute).toBeLessThanOrEqual(hhmmToMinutes('16:00'));
  });
  ...
  it('prefers all-three meetups and clear landmarks', () => {
    ...
    const sels = [sel('robbie', 'r2'), sel('ari', 'a2'), sel('morgan', 'm2')];
    const meetups = findMeetups('saturday', makeCtx(perfs, sels));
