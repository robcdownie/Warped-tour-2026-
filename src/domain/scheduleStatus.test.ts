...
describe('schedule day status (plan §P0-1)', () => {
  ...
  it('is complete when a human marked the day verified, even if partial', () => {
    ...
    const info = dayScheduleInfo(
      'saturday',
      perfs,
      prov({ saturdayVerifiedAt: '2026-07-25T11:08:00Z', saturdayVerifiedBy: 'Robbie' }),
    );
    expect(info.status).toBe('complete');
    expect(info.verifiedBy).toBe('Robbie');
  });
  ...
  it('counts selected sets that have no time as unknown', () => {
    ...
    const sels = [
      { userId: 'robbie', performanceId: 'a', selected: true },
      { userId: 'robbie', performanceId: 'b', selected: true },
      { userId: 'robbie', performanceId: 'c', selected: false },
      { userId: 'ari', performanceId: 'b', selected: true },
    ];
    expect(unknownSelectedCount('saturday', perfs, sels, 'robbie')).toBe(1);
  });
});
