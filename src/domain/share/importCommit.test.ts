...
function seedRepo(): FakeRepo {
  ...
  repo._state.users.set('robbie', user('robbie', 'Robbie'));
  repo._state.users.set('ari', user('ari', 'Ari'));
...
describe('selections import round-trip', () => {
  it('encode → decode → preview → commit replaces the friend selections, rollback restores', async () => {
    const repo = seedRepo();
    // Ari previously shared p1 + p3; the new code has p1 (updated) + p2 only.
    repo._state.selections.set(selKey('ari', 'p1'), sel('ari', 'p1'));
    repo._state.selections.set(selKey('ari', 'p3'), sel('ari', 'p3', { priority: 'must-see' }));

    const incoming: Selection[] = [
      sel('ari', 'p1', { priority: 'must-see' }),
      sel('ari', 'p2'),
    ];
    const code = encodeSelections(user('ari', 'Ari'), incoming, NOW);
    ...
    const ariSel = (await repo.allSelections()).filter((s) => s.userId === 'ari');
    expect(ariSel.map((s) => s.performanceId).sort()).toEqual(['p1', 'p2']);
    expect(ariSel.find((s) => s.performanceId === 'p1')?.priority).toBe('must-see');
    // Import metadata stamped.
    expect(repo._state.settings.friendImports['ari']?.selectionCount).toBe(2);

    // Rollback restores the pre-import world, including the removed p3.
    const ok = await rollbackImport(asRepo(repo), backupId);
    expect(ok).toBe(true);
    const restored = (await repo.allSelections()).filter((s) => s.userId === 'ari');
  ...
  it('skips selections that reference unknown performances', async () => {
    const repo = seedRepo();
    const code = encodeSelections(user('ari', 'Ari'), [sel('ari', 'p1'), sel('ari', 'ghost-perf')], NOW);
    const env = decodeEnvelope(code);
    await commitImport(asRepo(repo), env);
    const ariSel = (await repo.allSelections()).filter((s) => s.userId === 'ari');
    expect(ariSel.map((s) => s.performanceId)).toEqual(['p1']);
  });
});
...
describe('schedule import', () => {
  it('applies times and preserves a confirmed status when nothing changed', async () => {
    ...
    const code = encodeSchedule(await repo.allPerformances(), 'robbie', NOW);
  ...
  it('recomputes status when the import actually changes a set', async () => {
    ...
    const code = encodeSchedule(changed, 'robbie', NOW);
    await commitImport(asRepo(repo), decodeEnvelope(code));
...
describe('backup restore', () => {
  it('replaces users/selections/checkins wholesale and restores settings (keeping local offlineReady)', async () => {
    ...
    repo._state.selections.set(selKey('robbie', 'p3'), sel('robbie', 'p3'));
    repo._state.checkins.set('c-old', { id: 'c-old', userId: 'robbie', locationId: null, customCoordinates: null, source: 'manual', updatedAt: NOW });
    await repo.putSettings({ ...DEFAULT_SETTINGS, offlineReady: true, minMeetupMinutes: 15 });

    const backup: BackupData = {
      users: [user('robbie', 'Robbie'), user('ari', 'Ari')],
      selections: [sel('robbie', 'p1')],
      performances: [perf('p1'), perf('p2'), perf('p3')],
      locations: [],
      checkins: [],
      settings: { ...DEFAULT_SETTINGS, offlineReady: false, minMeetupMinutes: 25 },
    };
    const env = decodeEnvelope(encodeBackup(backup, 'robbie', NOW));
    ...
    expect((await repo.allUsers()).map((u) => u.id).sort()).toEqual(['ari', 'robbie']); // stray gone
    ...
    expect((await repo.allUsers()).map((u) => u.id).sort()).toEqual(['ari', 'robbie', 'stray']);
