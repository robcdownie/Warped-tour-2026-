...
describe('share codec', () => {
  it('round-trips a payload', () => {
    const data = { hello: 'world', list: [1, 2, 3], u: 'robbie' };
    const code = encodeEnvelope('selections', 'robbie', data, '2026-07-24T12:00:00Z');
    const env = decodeEnvelope(code);
    expect(env.type).toBe('selections');
    expect(env.source).toBe('robbie');
  ...
  it('detects corruption via checksum', () => {
    const code = encodeEnvelope('schedule', 'robbie', { p: [['x', 'y', '12:00', null]] }, 'now');
