...
const sel = (pid: string): Selection => ({
  userId: 'robbie', performanceId: pid, priority: 'must-see',
...
describe('leave-by planning (add-on §2)', () => {
  it('leave-by is the set start minus the walk', () => {
    const info = nextLeaveBy('robbie', 'saturday', hhmmToMinutes('15:50'), ctx)!;
  ...
  it('counts down to LEAVING, not to the set starting', () => {
    ...
    const info = nextLeaveBy('robbie', 'saturday', at, ctx)!;
  ...
  it('skips sets that have already started', () => {
    const plan = leaveByPlan('robbie', 'saturday', hhmmToMinutes('15:20'), ctx, 5);
  ...
  it('says "likely late" when the current set runs past the leave-by moment', () => {
    ...
    const info = nextLeaveBy('robbie', 'saturday', hhmmToMinutes('15:30'), tightCtx)!;
    expect(info.urgency).toBe('late');
  });

  it('uses the entrance as the origin before the first set', () => {
    const info = nextLeaveBy('robbie', 'saturday', hhmmToMinutes('11:30'), ctx)!;
  ...
  it('a heavier crowd setting means leaving earlier', () => {
    ...
    const light = nextLeaveBy('robbie', 'saturday', at, { ...ctx, crowd: 'light' })!;
    const heavy = nextLeaveBy('robbie', 'saturday', at, { ...ctx, crowd: 'heavy' })!;
  ...
  it('ignores sets the user is skipping', () => {
    ...
    expect(nextLeaveBy('robbie', 'saturday', hhmmToMinutes('15:50'), skipping)).toBeNull();
  });
...
describe('what the leave-by card can honestly say', () => {
  ...
  it('reports the earliest you could actually go, not just the clock', () => {
    ...
    const info = leaveByPlan('robbie', 'saturday', hhmmToMinutes('18:05'), tightCtx)[0];
  ...
  it('costs nothing when the gap covers the walk', () => {
    const info = leaveByPlan('robbie', 'saturday', hhmmToMinutes('15:50'), ctx)[0];
  ...
  it('admits when a stage is missing instead of implying a zero-minute walk', () => {
    ...
    const info = leaveByPlan('robbie', 'saturday', hhmmToMinutes('18:05'), {
      ...ctx,
      selections: [sel('a'), sel('b')],
